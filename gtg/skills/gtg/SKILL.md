...
| "gtg &lt;verb&gt;" not listed above (e.g. "gtg stats", "gtg issues") | Run `gtg.mjs <verb>` and relay its output. Bundled extras ship with the plugin (`stats` — a handoff-store snapshot); a `<storage-root>/.gtg/commands/<verb>.mjs` you've added resolves here too (yours overrides a bundled one of the same name). Unknown → the CLI errors. Stop. **Exception:** if the first output line starts with `GTG-DIRECTIVE:`, don't relay it — follow the instruction on that line instead. Custom commands use this to hand control back to the skill (e.g. to the Resume Procedure) instead of just printing. |
