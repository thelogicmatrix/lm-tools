...
// Reads BOTH stores: an entry idle >7d is auto-shelved onto the backlog by
...
export default ({ args, readStore, countHandoffFiles }) => {
  const learning = (rel, key) => {
    const store = readStore(rel);
    const all = Array.isArray(store?.[key]) ? store[key].filter(Boolean) : [];
    return all.filter((e) => e.parent === 'learning').sort((a, b) => String(a.project).localeCompare(String(b.project)));
  };
  const active = learning('docs/handoffs/_active.json', 'handoffs');
  const shelved = learning('docs/handoffs/_backlog.json', 'backlog');
