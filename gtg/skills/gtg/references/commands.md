...
| "gtg peek &lt;project&gt;" | Read `docs/handoffs/active/&lt;slug&gt;.json` (or `docs/handoffs/backlog/&lt;slug&gt;.json`) — one file, one entry — then read its `file` verbatim and relay the content. Don't know the slug? `gtg.mjs list` prints it. **Never read `_active.json` / `_backlog.json`**: those are the packed files the store replaced, frozen at the migration, so they serve a stale handoff path for anything touched since. **Do not consume**, no store mutation. |
